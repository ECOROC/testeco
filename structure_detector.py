...
VOCABULAIRE = [
    ...
    ("unite", [
        "unité", "unite", "u.", "unité de mesure", "unit", "u"
    ]),
    ("quantite", [
...
def _matcher_entetes_semantique(row: tuple) -> dict:
    ...
    for idx, cell in enumerate(row):
        ...
        for role, fragments in VOCABULAIRE:
            if role in mapping:
                # Exception : si on voit une 2e colonne quantité, la garder comme quantite_ref
                if role == "quantite":
                    for fragment in fragments:
                        if fragment in cell_str:
                            # C'est une 2e colonne quantité → stocker comme ref
                            if "quantite_ref" not in mapping:
                                mapping["quantite_ref"] = idx
                            break
                continue
            for fragment in fragments:
                if fragment in cell_str:
    ...
    # Si on a quantite_ref mais pas quantite, promouvoir
